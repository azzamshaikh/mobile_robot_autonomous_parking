...
class Planner:
    ...
    def __init__(self,start, end, obstacles):
        ...
        self.closed_list = []
        self.open_list.put((self.start_node.f, self.start_node.h, self.start_node.g, self.start_node))
    ...
    def __call__(self, screen):
        f, h, g, current_node = self.open_list.get()
        ...
        if self.iterations != 0:
            if (self.angular_diff(current_node.state[2],self.goal_node.state[2]) < 0.15 and
                    distance(current_node.state, self.goal_node.state) < self.pixels_per_meter*2):
                # print('found')
        ...
        for motion in self.motion_primitives:
            ...
            if self.is_not_valid(screen,new_state) or abs(theta_new - theta_trailer_new) > np.deg2rad(45):
                ...
                new_node.g = round(current_node.g + angular_cost(new_node.state[2], self.goal_node.state[2]) +
                                   cost(current_node.state, new_node.state),2)
                new_node.f = round(new_node.g + new_node.h,2)
                children.append(new_node)

        for child in children:
            if child not in self.closed_list and not any([node == child for f, h, g, node in self.open_list.queue]):
                self.open_list.put((child.f, child.h, child.g, child))
